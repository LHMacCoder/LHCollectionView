...
@interface LHImageFile (Internals)
...
+ (NSOperationQueue *)previewLoadingOperationQueue {
    ...
    if (queue == nil) {
        ...
        queue.name = @"TSImageFile Preview Loading Queue";
    }
    return queue;
}
...
+ (NSDictionary *)demoTagNamesDictionary {
    ...
    if (demoTagNames == nil) {
        demoTagNames = @{
                         ...
                         @"Flower 7" : @[@"Landscape", @"Snow", @"Water"],
                         @"Flower 2" : @[@"Landscape", @"Snow", @"Water"]
        ...
        [[[self class] previewLoadingOperationQueue] addOperationWithBlock:^{
            if ([self createImageSource]) {
                NSDictionary *options = [[NSDictionary alloc] initWithObjectsAndKeys:
                                         ...
                                         nil];
                CGImageRef thumbnail = CGImageSourceCreateThumbnailAtIndex(self->imageSource, 0, (CFDictionaryRef)options);
                if (thumbnail) {

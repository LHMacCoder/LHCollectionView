...
{
  if ([self.delegate respondsToSelector:@selector(collectionView:nameOfItem:startsWith:)]) {
    ...
    self.accumulatedKeyStrokes = [[self.accumulatedKeyStrokes stringByAppendingString:aString] lowercaseString];
    
    NSInteger firstIndex = [self.contentArray indexOfObjectWithOptions:NSEnumerationConcurrent passingTest:^BOOL(id obj, NSUInteger idx, BOOL *stop) {
      return [self.delegate collectionView:self nameOfItem:obj startsWith:self.accumulatedKeyStrokes];
    }];
    if (firstIndex != NSNotFound) {
      ...
      if (NSHeight([self frame]) > NSHeight([self visibleRect])) {
        ...
        [clipView scrollToPoint:NSMakePoint(0, MIN(NSHeight([self frame])-NSHeight([self visibleRect]),[self.layoutManager rectOfItemAtIndex:firstIndex].origin.y))];
...
- (void)simpleSelectItemAtIndex:(NSUInteger)anIndex
{
  if (anIndex != NSNotFound) {
    ...
    [self scrollRectToVisible:[self.layoutManager rectOfItemAtIndex:anIndex]];
  }
}
...
- (void)simpleExtendSelectionRange:(NSRange)range newIndex:(NSUInteger)newIndex
{
  if (newIndex != NSNotFound) {
    ...
    [self scrollRectToVisible:[self.layoutManager rectOfItemAtIndex:newIndex]];
  }
}
...
{
  NSPoint position = [self.layoutManager rowAndColumnPositionOfItemAtIndex:lastSelectionIndex];
  [self simpleSelectItemAtIndex:[self.layoutManager indexOfItemAtRow:position.y-1 column:position.x]];
}

- (void)moveUpAndModifySelection:(id)sender
{
  NSPoint position = [self.layoutManager rowAndColumnPositionOfItemAtIndex:lastSelectionIndex];
  NSUInteger newIndex = [self.layoutManager indexOfItemAtRow:position.y-1 column:position.x];
  if (newIndex == NSNotFound)
    newIndex = 0;
...
- (void)moveDownAndModifySelection:(id)sender
{
  ...
  NSUInteger newIndex = [self.layoutManager indexOfItemAtRow:position.y+1 column:position.x];
  if (newIndex == NSNotFound)
    newIndex = [self.contentArray count]-1;
...
{
  if ([self.delegate respondsToSelector:@selector(collectionView:deleteItemsAtIndexes:)])
    [self.delegate collectionView:self deleteItemsAtIndexes:selectionIndexes];
}
